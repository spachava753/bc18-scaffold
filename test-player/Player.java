        ...
        while (true) {
            ...
            for (long i = 0; i < vecUnits.size(); i++) {
                ...
                if (currentUnit.unitType() == UnitType.Worker) {
                    // replicating code
                    if (workerMap.size() < 125) {
                        if (currentUnit.unitType() == UnitType.Worker) {
                            for (Direction direction : Direction.values()) {
                                if (replicate(currentUnit, direction)){
                                    System.out.println("Replicated this round");
                                    break;
                                }
                            }
                        }
            ...
            System.out.println("workerMap size " + workerMap.size());

            int counter = 0;
            for (Unit unit : workerMap.values()) {
                if (unit == null) {
                    counter++;
                }
            }
            System.out.println("health is zero: " + counter);
